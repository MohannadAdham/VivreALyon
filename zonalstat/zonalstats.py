...
def zonal_stats(feat, input_zone_polygon, input_value_raster):
    ...
    xoff = abs(int((xOrigin - xmin) / pixelWidth))
    yoff = abs(int((yOrigin - ymax) / pixelHeight))
    ...
    dataraster = banddataraster.ReadAsArray(0, 0, xcount, ycount).astype(numpy.float)
...
def loop_zonal_stats(input_zone_polygon, input_value_raster):
    ...
    featList = []
    for feature in lyr:
        featList.append(feature.GetField('id'))
